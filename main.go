...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// allows front end to send request to check access-control-allow-origin
	if r.Method == "OPTIONS" {
		return
	}
	...
	// parse form data and create Post object
	...
	// get image file and save to GCS
	...
	// annotate image with vision api (using image's internal name)
	if p.Type == "image" {
...
func saveToGCS(r io.Reader, objectName string) (string, error) {
	...
	// check bucket exists by reading bucket attributes
	...
	// Set object's access control
	if err := object.ACL().Set(ctx, storage.AllUsers, storage.RoleReader); err != nil {
		return "", err
	}

	// get object attributes to see if object has been created
